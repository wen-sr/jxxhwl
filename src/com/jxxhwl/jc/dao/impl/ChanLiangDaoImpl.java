...
public class ChanLiangDaoImpl extends BaseDao implements ChanLiangDao{
	...
	public List<ChanLiang> loadData(int currentPage, int pageSize, String dd) {
		...
		List<ChanLiang> list = this.getJdbcTemplate().query(sql, new RowMapper() {
			...
			public ChanLiang mapRow(ResultSet rs, int arg1) throws SQLException {
				...
				c.setJiaobao(rs.getInt("JiaoBao"));
				return c;
			}
		});
		return list;
	...
	public int add(ChanLiang cl) {
		String sql = "insert into JiaoCaiChanLiang(LingPiaoZong,YunShuZong,YunShuDang,WeiDao,DaBao,TieBao,JiaoCan,WenJiao,NanChangShi,DaiFa,Tui,HeJi,dd,JiaoBao,adddate,addwho,note,issue) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,getdate(),?,?,?)";
		return this.getJdbcTemplate().update(sql, cl.getLingPiaoZong(),cl.getYunShuZong(),cl.getYunShuDang(),cl.getWeiDao(),cl.getDaBao(),cl.getTieBao(),cl.getJiaoCan(),cl.getWenJiao(),cl.getNanChangShi(),cl.getDaiFa(),cl.getTui(),cl.getHeJi(),cl.getDd(),cl.getJiaobao(),cl.getAddwho(),cl.getNote(),cl.getIssue());
	}
	/**
	 * 修改
	 */
	...
	public int edit(ChanLiang cl) {
		String sql = "update JiaoCaiChanLiang set LingPiaoZong=?,YunShuZong=?,YunShuDang=?,WeiDao=?,DaBao=?,TieBao=?,JiaoCan=?,WenJiao=?,NanChangShi=?,DaiFa=?,Tui=?,HeJi=?,dd=?,note=?,issue=?,jiaobao=? where id = ?";
		return this.getJdbcTemplate().update(sql, cl.getLingPiaoZong(),cl.getYunShuZong(),cl.getYunShuDang(),cl.getWeiDao(),cl.getDaBao(),cl.getTieBao(),cl.getJiaoCan(),cl.getWenJiao(),cl.getNanChangShi(),cl.getDaiFa(),cl.getTui(),cl.getHeJi(),cl.getDd(),cl.getNote(),cl.getIssue(),cl.getJiaobao(),cl.getId());
	}
	/**
	 * 获取楼下汇总
	 */
